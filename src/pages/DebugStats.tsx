...
export default function DebugStats() {
  ...
  const [testResult, setTestResult] = useState<{ data: any; error: any } | null>(null);
  ...
  useEffect(() => {
    ...
    const loadDebugData = async () => {
      try {
        ...
        console.log('Loading debug data for user:', user.user_id);
        ...
        // Get medications
        const { data: medsData, error: medsError } = await db.getMedications(user.user_id);
        if (medsError) {
        ...
        // Get intake logs
        const { data: logsData, error: logsError } = await db.getIntakeLogs(user.user_id);
        if (logsError) {
        ...
        if (!testResult && medsData && medsData.length > 0) {
          ...
          const { data: created, error: createError } = await db.createIntakeLog(med.id, testData);
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-2">Medications ({medications.length})</h2>
        <div className="space-y-2">
          {medications.map((med) => (
            <div key={med.id} className="p-3 bg-white rounded shadow">
              ...
              <div className="text-sm text-gray-600">Active: {med.is_active ? 'Yes' : 'No'}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-2">Intake Logs ({intakeLogs.length})</h2>
        <div className="space-y-2">
          {intakeLogs.map((log) => (
            <div key={log.id} className="p-3 bg-white rounded shadow">
              <div className="font-medium">Medication ID: {log.medication_id}</div>
              <div className="text-sm text-gray-600">Scheduled: {new Date(log.scheduled_time).toLocaleString()}</div>
              <div className="text-sm text-gray-600">Taken: {log.taken_at ? new Date(log.taken_at).toLocaleString() : 'Not taken'}</div>
              <div className="text-sm text-gray-600">Status: {log.status}</div>
              <div className="text-sm text-gray-600">Notes: {log.notes || 'No notes'}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          {testResult ? (
            testResult.error ? (
              <>
                ...
                <pre className="mt-2 overflow-auto">{JSON.stringify(testResult.error, null, 2)}</pre>
              </>
            ) : (
              <>
                <div className="text-green-600 font-medium">Insert success</div>
                <pre className="mt-2 overflow-auto">{JSON.stringify(testResult.data, null, 2)}</pre>
              </>
            )
          ) : (
      ...
      <div className="mb-8">
        ...
        <div className="space-y-2">
          {medications.map((med) => {
            const medLogs = intakeLogs.filter(log => log.medication_id === med.id);
            return (
              <div key={med.id} className="p-3 bg-white rounded shadow">
                <div className="font-medium">{med.generic_name}</div>
                <div className="text-sm text-gray-600">Logs: {medLogs.length}</div>
                <div className="text-sm text-gray-600">Taken: {medLogs.filter(log => log.taken_at).length}</div>
              </div>
            );
          })}
        </div>
