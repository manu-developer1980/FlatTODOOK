...
export default function SubscriptionManager() {
  ...
  const handleCancelSubscription = async () => {
    ...
    if (!confirm('¿Estás seguro de que quieres cancelar tu suscripción premium? Tu plan cambiará a gratuito al final del período de facturación actual.')) {
      return
    }
    ...
    try {
      ...
      if (updatedSubscription) {
        toast.success('Suscripción cancelada exitosamente. Tu plan cambiará a gratuito al final del período actual.')
  ...
  const isPremiumCanceled = subscription?.plan === 'premium' && subscription?.status === 'canceled'

  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Suscripción</h1>
        <p className="text-gray-600">Gestiona tu plan de suscripción en MediTrack</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={`p-3 rounded-full ${isPremiumActive ? 'bg-yellow-100' : 'bg-gray-100'}`}>
              <Crown className={`h-6 w-6 ${isPremiumActive ? 'text-yellow-600' : 'text-gray-600'}`} />
            </div>
            <div>
              ...
              <p className="text-gray-600">
                {isPremiumActive ? 'Tienes acceso completo a todas las funciones' : 'Acceso limitado a funciones básicas'}
              </p>
            </div>
          </div>
            ...
            {subscription.cancel_at_period_end && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-800">
                  Tu suscripción se cancelará el {formatDate(subscription.current_period_end)} y cambiará al plan gratuito.
                </p>
              </div>
            )}
          </div>
        ...
        {isPremiumActive ? (
          <div className="space-y-4">
            <button
              ...
              disabled={isLoading || isProcessing || subscription?.cancel_at_period_end}
            ...
            >
              ...
              {subscription?.cancel_at_period_end ? 'Cancelación programada' : 'Cancelar suscripción'}
            </button>
            {subscription?.cancel_at_period_end && (
              <p className="text-sm text-gray-600">
                Tu suscripción ya está programada para cancelarse. Contacta con soporte si necesitas ayuda.
              </p>
            )}
          </div>
        ) : (
